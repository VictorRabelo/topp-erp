...
@Component({
	selector: 'kt-conta-receber-payment',
	templateUrl: './conta-receber-payment.component.html',
	styleUrls: ['./conta-receber-payment.component.scss']
})
export class ContaReceberPaymentComponent implements OnInit {

	contas = [];
	dados: any = { descricao: 'Recebimento de conta' };
	...
	receber() {
		...
		this._loading(true);
		this.service.paymentContaReceber(request).subscribe((resp: any) => {
			this._loading();
			this.close(true);
		}, erro => {
			this._loading();
