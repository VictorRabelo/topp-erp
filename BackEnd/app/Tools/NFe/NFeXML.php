...
class NFeXML
{
   ...
   private function gen_itens()
   {
      ...
      foreach ($this->items as $i => $item) {
         ...
         $std->vProd = number_format($item['valor_unitario'], 2, '.', '');
   ...
   public function getPDF(object $dados)
   {
      ...
      $pathPDF = "{$dados->cnpj}/pdf/nfe/{$mesPath}/{$tpambPath}/{$dados->chave}.pdf";
   ...
   private function geraPDF(object $dados)
   {
      ...
      $pathXML = "{$dados->cnpj}/xml/nfe/autorizados/{$mesPath}/{$tpambPath}/{$dados->chave}.xml";
